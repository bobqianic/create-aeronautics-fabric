...
public class ServerSubLevel extends SubLevel implements PhysicsPipelineBody {
    ...
    public ServerSubLevel(final ServerLevel level, final int plotX, final int plotY, final Pose3d pose) {
        ...
        // A newly allocated sub-level is registered with the physics pipeline before
        // blocks are moved into (or loaded into) its plot. Give the pipeline a valid,
        // empty tracker immediately; block-change callbacks populate it afterwards.
        this.massTracker = new MergedMassTracker(this, new MassTracker());
