...
public abstract class AbstractSableMixinPlugin implements IMixinConfigPlugin {
    ...
    public void onLoad(final String mixinPackage) {
        ...
        final boolean sodiumLoaded = SableLoaderPlatform.INSTANCE.isModLoaded("sodium");
        LOGGER.info("Using {} renderer mixins", sodiumLoaded ? "Vanilla with Sodium compatibility" : "Vanilla");
    }
    ...
    public boolean shouldApplyMixin(final String targetClassName, final String mixinClassName) {
        ...
        if (mixinClassName.startsWith("dev.ryanhcode.sable.mixin.sublevel_render.impl")) {
            if (mixinClassName.startsWith("dev.ryanhcode.sable.mixin.sublevel_render.impl.sodium")) {
                return this.modLoadedCache.computeIfAbsent("sodium", x -> SableLoaderPlatform.INSTANCE.isModLoaded("sodium"));
            }
            return mixinClassName.startsWith("dev.ryanhcode.sable.mixin.sublevel_render.impl.vanilla");
        }

        // ScalableLux owns this light-packet queue call at a higher mixin
        // priority and applies packet light data synchronously. Plot chunks
        // are supplied by the shared StarLightInterface compatibility mixin.
        if (mixinClassName.equals("dev.ryanhcode.sable.mixin.plot.lighting.ClientPacketListenerMixin")
                && this.modLoadedCache.computeIfAbsent(
                        "scalablelux",
                        x -> SableLoaderPlatform.INSTANCE.isModLoaded("scalablelux")
                )) {
            return false;
        }
