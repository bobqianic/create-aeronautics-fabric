...
public interface BlockEntityLiftingGasProvider {
    ...
    /**
     * These accessors intentionally do not use the vanilla {@code BlockEntity} method names.
     * Vanilla-owned method names are remapped at runtime, while methods declared by this
     * interface are not; redeclaring {@code getBlockPos()} or {@code getLevel()} here causes
     * an {@link AbstractMethodError} in production.
     */
    BlockPos getProviderBlockPos();
    ...
    default void tickBalloonLogic() {
        ...
        if (this.getBalloon() instanceof final ServerBalloon balloon && balloon.getTotalFilledVolume() > 1) {
            AeroAdvancements.HEAD_IN_THE_CLOUDS.awardToNearby(this.getProviderBlockPos(), this.getProviderLevel());
        }
    }
    ...
    record ClientBalloonInfo(int clientBalloonVolume, double clientBalloonFilled,
        ...
        public static void writeToNBT(final ValueOutput tag, final ServerBalloon balloon) {
        ...
        public static ClientBalloonInfo readFromNBT(final ValueInput tag) {
            return new ClientBalloonInfo(
                    ...
                    tag.getDoubleOr("Filled", 0),
                    tag.getDoubleOr("Target", 0),
                    tag.getDoubleOr("Lift", 0),
                    tag.getDoubleOr("Delta", 0),
                    new Vec3(
                            tag.getDoubleOr("CenterX", 0),
                            tag.getDoubleOr("CenterY", 0),
                            tag.getDoubleOr("CenterZ", 0)
                    ));
        }
    }
}
