...
public class SpringItemHandler implements InteractCallback {
    ...
    public void clientTick(final Level level, final LocalPlayer player) {
        // ItemEntry implements Holder, but it is a wrapper rather than the registry's
        // canonical Holder.Reference. ItemStack#is(Holder) compares holder identity,
        // so passing the entry itself always reports false on Fabric.
        ...
        if (this.linkPos != null) {
            ...
            Outliner.getInstance().showOutline(this.linkPos + "Spring", new SubLevelAABBOutline(linkAABB))
            ...
            if (clientHit != null && clientHit.getType() != HitResult.Type.MISS && clientHit instanceof final BlockHitResult hit) {
                ...
                final DustParticleOptions data = new DustParticleOptions(color, 1);
                ...
                Outliner.getInstance().showOutline(this.linkPos + " Spring Selection", new SubLevelAABBOutline(hitAABB))
                        .colored(color)
                        .lineWidth(1 / 16f);
            }
        }
