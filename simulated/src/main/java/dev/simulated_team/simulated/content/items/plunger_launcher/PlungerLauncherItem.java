...
import com.zurrtum.create.foundation.item.CustomArmPoseItem;
...
public class PlungerLauncherItem extends Item implements CustomArmPoseItem {
    ...
    public InteractionResult use(final Level level, final Player player, final InteractionHand interactionHand) {
        ...
        if (!level.isClientSide()) {
            ...
            // send shoot packet
            VeilPacketManager.player((ServerPlayer) player).sendPacket(new PlungerLauncherShootPacket(interactionHand, reload));
            // todo: send to all tracking players for playing (to be implemented) external animation
//            VeilPacketManager.tracking(player).sendPacket(new PlungerLauncherShootPacket(interactionHand, reload));
    ...
    public @NotNull BarrelAndCorrectionInfo getCorrectionInfo(final Player player, final InteractionHand interactionHand) {
		...
		final BlockHitResult hit = level.clip(new ClipContext(
			eyePos,
			eyePos.add(player.getLookAngle().scale(48)),
			ClipContext.Block.COLLIDER,
			ClipContext.Fluid.NONE,
