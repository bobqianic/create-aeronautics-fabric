...
//            }
    ...
    public static void renderFBO(final GuiGraphics graphics, final AdvancedFbo fbo, final int width, final int height) {
        ...
        pose.scale(1, -1);
        graphics.fill(RenderPipelines.GUI_TEXTURED, TextureSetup.singleTexture(fbo.getColorTextureAttachment(0).getTextureView()), 0, 0, width, height);
        pose.popMatrix();
    }
    ...
    private static void addForceArrowTooltip(final ForceGroup forceGroup, final int forceCount, final double forceMagnitude, final int color, final List<FormattedText> tooltipLines) {
        final CommonLangBuilder forceNameText = SimLang.builder().add(forceGroup.name()).color(color);
        final CommonLangBuilder forceMagnitudeText = SimLang.translate("contraption_diagram.force_arrow_magnitude", String.format("%,.2f", forceMagnitude)).color(0xffffffff);
