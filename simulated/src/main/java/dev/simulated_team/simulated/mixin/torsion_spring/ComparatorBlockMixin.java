...
public class ComparatorBlockMixin {
    ...
    private int simulated$potentiallyDirectionalAnalogueSignal(final BlockState instance, final Level level, final BlockPos pos, final Direction side, final Operation<Integer> original) {
        if (instance.getBlock() instanceof final IDirectionalAnalogOutput directionalAnalogOutput) {
            return directionalAnalogOutput.getAnalogOutputSignalFrom(instance, level, pos, side.getOpposite());
        }
        return original.call(instance, level, pos, side);
    }
}
