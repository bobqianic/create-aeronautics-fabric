...
public class SimItems {
    ...
    public static final ItemEntry<PhysicsStaffItem> PHYSICS_STAFF =
            REGISTRATE.item("creative_physics_staff", PhysicsStaffItem::new)
                    ...
                    .model((ctx, provider) -> {
                        provider.withExistingParent(ctx.getName(), Simulated.path("item/creative_physics_staff/item"));
                        provider.customItemModel(ctx.getName(), Simulated.path("model/creative_physics_staff"));
                    })
                    .register();
    ...
    public static final ItemEntry<PlungerLauncherItem> PLUNGER_LAUNCHER =
            REGISTRATE.item("plunger_launcher", PlungerLauncherItem::new)
                    ...
                    .model((ctx, provider) -> {
                        provider.withExistingParent(ctx.getName(), Simulated.path("item/plunger_launcher/item"));
                        provider.customItemModel(ctx.getName(), Simulated.path("model/plunger_launcher"));
                    })
                    .tag(ItemTags.ARMOR_ENCHANTABLE, ItemTags.DURABILITY_ENCHANTABLE)
                    .register();
    ...
    private static ItemBuilder<Item, Registrate> ingredientNoRegister(final String name) {
        return REGISTRATE.item(name, Item::new);
    }
