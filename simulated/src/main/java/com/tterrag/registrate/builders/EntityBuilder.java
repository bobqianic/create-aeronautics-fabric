...
public class EntityBuilder<T extends Entity, P> extends AbstractBuilder<EntityType<?>, EntityType<T>, P, EntityBuilder<T, P>> {
     ...
     * @return this {@link EntityBuilder}
     */
    public EntityBuilder<T, P> renderer(NonNullSupplier<NonNullFunction<EntityRendererProvider.Context, EntityRenderer>> renderer) {
        this.renderer = renderer;
        return this;
    }

    protected void registerRenderer() {
        EntityRendererRegistry.register(getEntry(), context -> renderer.get().apply(context));
    }
    ...
    public ItemBuilder<? extends SpawnEggItem, EntityBuilder<T, P>> spawnEgg(int primaryColor, int secondaryColor) {
        return getOwner().<SpawnEggItem, EntityBuilder<T, P>>item(this, getName() + "_spawn_egg", SpawnEggItem::new).tab(CreativeModeTabs.SPAWN_EGGS)
    ...
    protected EntityType<T> createEntry() {
        ...
        if (renderer != null) {
            EnvExecutor.runWhenOn(EnvType.CLIENT, () -> () ->
                    EntityRendererRegistry.register(type, context -> renderer.get().apply(context)));
        }
        return type;
    }
@Deprecated
