...
public class BlockBuilder<T extends Block, P> extends AbstractBuilder<Block, T, P, BlockBuilder<T, P>> {
     ...
     * @return this {@link BlockBuilder}
     */
    public BlockBuilder<T, P> initialProperties(NonNullSupplier<?> propertiesOrBlock) {
        initialProperties = () -> {
            Object value = propertiesOrBlock.get();
            if (value instanceof Block block) {
                return BlockBehaviour.Properties.ofFullCopy(block);
            }
            if (value instanceof BlockBehaviour.Properties properties) {
                return properties;
            }
            throw new IllegalArgumentException("Initial block properties supplier returned " + value);
        };
        return this;
    }

    public BlockBuilder<T, P> addLayer(Supplier<Supplier<?>> layer) {
        if (renderLayer == null) {
